...
limits. As an example, suppose we want to show that[^6] $$\nonumber
\lim_{n\to\infty} \int_{0}^\infty \frac{e^{-nx^2}}{\sqrt{x}} \,\mathrm{d}x
=
0$$ or $$\nonumber
...
by \@ref(eq:defexp) but instead $$\label{eq:expcompute}
\mathbb{E}X = \int_\mathbb{R}x \, \mathbb{P}_X(\mathrm{d}x),$$ which is
a particular case of the above theorem with $f=X$ and $g(x)=x$.
:::
...
Remark [\[rmk:intsimpleunique\]](#rmk:intsimpleunique){reference-type="ref"
reference="rmk:intsimpleunique"}, we see that any other definition of
integral of non-negative extended Borel functions that is unitary,
