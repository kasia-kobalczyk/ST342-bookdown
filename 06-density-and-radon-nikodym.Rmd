...
sense. Given a measure $\nu$, a $\sigma$-finite measure $\mu$, and two
extended Borel functions $f$ and $g$ such
that \@ref(eq:radon) holds, then $f = g$ for $\mu$-a.e. $\omega$.
:::
...
:::{.exercise}
Suppose that two extended random variables $Z$ and $W$
satisfy \@ref(eq:condexp1)
and \@ref(eq:condexp2). Prove that $\mathbb{P}(Z=W)=1$.
:::

:::{.remark}
The above definition says *the* conditional expectation because, if
there is such a function, it is unique in the a.s. sense. Conversely, if
$Z$ satisfies \@ref(eq:condexp2) and $W = Z$ a.s., then $W$ also
satisfies \@ref(eq:condexp2).
:::
...
::: {.proof name="non-negative integrable case"}
